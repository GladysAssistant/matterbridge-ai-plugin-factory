 ...
 */
import { MatterbridgeDynamicPlatform, MatterbridgeEndpoint, MatterbridgeThermostatServer, airConditioner, powerSource } from 'matterbridge';
import { MelCloudClient, melcloudModeToMatter, matterModeToMelcloud } from './melcloud.js';
...
export class MelCloudPlatform extends MatterbridgeDynamicPlatform {
    ...
    async registerMelCloudDevice(device) {
        ...
        const endpoint = new MatterbridgeEndpoint([airConditioner, powerSource], { id: uniqueId })
            ...
            .createDefaultBridgedDeviceBasicInformationClusterServer(deviceName, serial, this.matterbridge.aggregatorVendorId, 'Mitsubishi Electric', 'MELCloud AC')
            .createDefaultPowerSourceWiredClusterServer()
            .createDeadFrontOnOffClusterServer(device.Power)
            .createDefaultThermostatClusterServer(localTemp, heatSetpoint, coolSetpoint, 100, // minSetpointDeadBand 1°C
        minHeat, maxHeat, minCool, maxCool)
        ...
        endpoint.addCommandHandler('on', async () => {
            ...
            try {
                ...
                const mode = endpoint.getAttribute('thermostat', 'systemMode', this.log);
    ...
    async updateEndpoint(endpoint, state) {
        // Update OnOff
