 ...
 * @file module.ts
 */
import { MatterbridgeDynamicPlatform, MatterbridgeEndpoint, airConditioner, powerSource } from 'matterbridge';
...
export class MelCloudPlatform extends MatterbridgeDynamicPlatform {
    ...
    async onStart(reason) {
        ...
        try {
            ...
            this.log.error(`MELCloud login failed: ${err.message}`);
        }
    }
    async onConfigure() {
        ...
        const pollSeconds = (this.config['pollInterval'] ?? 60) * 1000;
        this.pollInterval = setInterval(() => {
            this.pollDevices().catch((e) => this.log.error(`Poll error: ${e.message}`));
        }, pollSeconds);
    ...
    async discoverDevices() {
        ...
        try {
            ...
            this.log.error(`Failed to get MELCloud devices: ${err.message}`);
            return;
        }
        ...
        for (const dev of devices) {
            ...
            const endpoint = new MatterbridgeEndpoint([airConditioner, powerSource], { id: endpointId })
                ...
                .createDefaultThermostatClusterServer(localTempCentidegrees, // localTemperature in 0.01°C units
            setpointCentidegrees, // occupiedHeatingSetpoint
            setpointCentidegrees)
            ...
            endpoint.addCommandHandler('setpointRaiseLower', async (data) => {
                ...
                try {
                    ...
                    this.log.error(`Failed to set temperature: ${e.message}`);
                }
            });
    ...
    async pollDevices() {
        if (!this.client || this.deviceMap.size === 0)
            return;
        for (const [deviceId, endpoint] of this.deviceMap.entries()) {
            try {
                ...
                this.log.error(`Failed to poll device ${deviceId}: ${e.message}`);
            }
        }
    }
}
