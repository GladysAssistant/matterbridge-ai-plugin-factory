...
import { AjaxPlatform } from '../src/module.js';
import type { AjaxPlatformConfig } from '../src/types.js';
...
const mockMatterbridge: PlatformMatterbridge = {
  ...
  rootDirectory: path.join('.cache', 'jest', 'AjaxPlugin'),
  homeDirectory: path.join('.cache', 'jest', 'AjaxPlugin'),
  matterbridgeDirectory: path.join('.cache', 'jest', 'AjaxPlugin', '.matterbridge'),
  matterbridgePluginDirectory: path.join('.cache', 'jest', 'AjaxPlugin', 'Matterbridge'),
  matterbridgeCertDirectory: path.join('.cache', 'jest', 'AjaxPlugin', '.mattercert'),
  globalModulesDirectory: path.join('.cache', 'jest', 'AjaxPlugin', 'node_modules'),
...
const mockConfig: AjaxPlatformConfig = {
  name: 'matterbridge-ai-factory-ajax-systems-alarmes-et-domotique-s-curit-hub-d-tecteurs-sir-nes-relais-prises',
  type: 'DynamicPlatform',
  version: '1.0.0',
  whiteList: [],
  blackList: [],
  mode: 'api',
  exposeDemoDevices: true,
  debug: false,
...
describe('Ajax Systems Platform', () => {
  let instance: AjaxPlatform;
  ...
  it('should throw an error if matterbridge is not the required version', async () => {
    expect(() => new AjaxPlatform({ ...mockMatterbridge, matterbridgeVersion: '2.0.0' }, mockLog, mockConfig)).toThrow(
  ...
  it('should create an instance of the platform', async () => {
    instance = (await import('../src/module.js')).default(mockMatterbridge, mockLog, mockConfig) as AjaxPlatform;
    expect(instance).toBeInstanceOf(AjaxPlatform);
  ...
  it('should start and expose demo devices', async () => {
